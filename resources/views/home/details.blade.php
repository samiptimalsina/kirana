        ...
        <div class="flex flex-wrap -mx-2 mt-20 bg-white p-6 rounded-lg shadow-lg">
                            ...
                            @foreach ($product->img as $image)
                                <li data-thumb="{{ asset($image) }}">
                                    <img class="w-full rounded-lg " src="{{ asset($image) }}" alt="{{ $product->name }}"
                                        style="height: 362px; width:300px;" />
                                </li>
                            @endforeach
                        @else
                            <li data-thumb="{{ asset($product->img) }}">
                                <img class=" w-full rounded-lg" src="{{ asset($product->img) }}"
                                    alt="{{ $product->name }}" />
                            </li>
                        @endif
                    </ul>
            ...
            <div class="w-full md:w-7/12 px-2">
                <h3 class="font-bold font-cursive-merie text-4xl">
                    ...
                    <span
                        class="text-amber-400 group-hover:text-slate-600 p-1 text-right">Rs{{ $product->price }}</span>
                </h3>
                ...
                <form action="{{ route('reservation.product') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="food_id" value="{{ $product->id }}" readonly>
                    ...
                    <div class=" -mx-2 mb-4">
                        <div class="w-full md:w-1/2 px-2 mb-4 md:mb-0 custom-input">
                            ...
                            <input type="text" id="name" name="name"
                                ...
                                placeholder="Enter Receiver name" value="{{ old('name') }}">
                            @error('name')
                        ...
                        <div class="w-full md:w-1/2 px-2 custom-input my-2">
                            ...
                            <input type="text" id="phone" name="phone"
                                ...
                                focus:border-amber-500 focus:ring-amber-500"
                                placeholder="Enter Receiver Phone Number" required value="{{ old('phone') }}">
                            @error('phone')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="w-full md:w-1/2 px-2 custom-input my-2">
                            ...
                            <input type="text" id="address" name="address"
                                ...
                                focus:border-amber-500 focus:ring-amber-500"
                                placeholder="Enter Receiver Full Address" value="{{ old('address') }}">
                            @error('address')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex flex-wrap -mx-2 mb-4">
                        <div class="w-full md:w-1/2 px-2 custom-input">
                            ...
                            <select name="person" id="person"
                                ...
                                @for ($i = 1; $i <= 10; $i++)
                                    <option value="{{ $i }}" {{ old('person') == $i ? 'selected' : '' }}>
                                        {{ $i }}
                                    </option>
                                @endfor
                            </select>
                    ...
                    <div class="text-left">
                        <button type="submit"
                            class="inline-block rounded-sm shadow-md mt-7 px-5 py-3 mx-8 bg-amber-500 uppercase text-sm font-bold text-white border border-transparent transition ease-in-out duration-300 custom-button">
                            Book a Product
                        </button>
                    </div>
                </form>
